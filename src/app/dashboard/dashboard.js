...
export default function Dashboard(props) {
  ...
  if (!props.user.organization.admin) {
    return (
      <div className="flex flex-row items-center justify-center min-h-screen py-2">
        ...
        <p className="absolute bottom-4 right-4 text-primary">{props.subscriptionName}</p>
      </div >
    )
  } else {
    return (
      <div className="flex flex-row">
        <div className="flex flex-col space-y-4 text-[white] bg-base-300 sticky left-0 top-0 w-1/6 h-screen z-10 px-4 py-8">
          <h1 className="text-3xl font-semibold">{props.user.organization.displayName}</h1>
        ...
        <div className="flex flex-col space-y-4 text-[white] w-5/6">
          <h1
            className="text-3xl font-semibold text-center bg-primary p-4"
          >{selectedNavOption.toUpperCase()}</h1>
        </div>
      </div>
    )
  }

...
export async function GET(request) {
  ...
  let events = await Events.find({}).populate("accepted").populate("organizationId");

  // get all events that are happening tomorrow
  const tomorrow = new Date();
  tomorrow.setDate(tomorrow.getDate() + 1);
  events = events.filter((event) => {
    return new Date(event.date).setHours(0, 0, 0, 0) === tomorrow.setHours(0, 0, 0, 0);
  })

  // send reminders to all volunteers
  events.forEach((event) => {
    ...
    // send reminder to volunteers that have reminders notifications enabled
    const volunteers = event.accepted.filter((volunteer) => volunteer.notifications.reminders);
    ...
    if (volunteers.length === 0) {
      return new NextResponse({
        status: 200,
        body: "No volunteers to remind"
      });
    }

    sendBulkMail({
      to: volunteers.map((volunteer) => volunteer.email),
      from: `${organization.displayName} <notifications@manusunitas.com>`,
      replyTo: `${organization.displayName} <${organization.email}>`,
      subject: "Reminder: Volunteer Event Tomorrow",
      body: `
                          ...
                          <tr>
                            <td class="content-cell">
                              <div class="f-fallback">
                                You are scheduled to volunteer at an event tomorrow, ${format(new Date(event.date), "MMMM do")}. Here are the details:
                                <br>
                                <br>
